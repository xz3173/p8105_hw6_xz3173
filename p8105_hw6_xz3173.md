...
around 0.915. This high mean value suggests that, on average, the model
explains a large portion of the variance in the response variable `tmax`
using predictors `tmin` and `prcp`. The distribution of these
`r_squared` values is approximately across the bootstrap samples.**
