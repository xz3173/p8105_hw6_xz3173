...
**The distribution of `r_squared` values (coefficient of determination) from the 5000 bootstrap samples indicates a strong average model performance, with the mean `r_squared` around `r mean(r_squared)`, which indicates a better fit of the linear regression model to the data. This high mean value suggests that, on average, the model explains a large portion of the variance in the response variable `tmax` using predictors `tmin` and `prcp`. The distribution is quite narrow and peaks around the mean, indicating that most bootstrap samples provided a similar `r_squared` estimate, which suggests a strong model fit.**
...
```{r}
# Calculate the number of non-positive products including NAs
non_positive = sum(!bootstrap_results$beta_1 * bootstrap_results$beta_2 > 0, na.rm = TRUE)

# Calculate the number of positive prodcuts
positive = sum(bootstrap_results$beta_1 * bootstrap_results$beta_2 > 0, na.rm = TRUE)

# Calculate the total number of observations
total_observations = nrow(bootstrap_results)

# Calculate the proportion of non-positive or NA products
proportion_non_positive = non_positive / total_observations

# Print the proportion of non-positive or NA products
proportion_non_positive

# Create a tibble summarizing the counts
summary = 
  tibble(
...
**According to the result, there is about `r non_positive` of boot samples, the product of the coefficients for `tmin` and `prcp` was less than or equal to zero. Non-positive values for a product of coefficients suggest that one or both predictors have a negative association with the response variable `tmax` in a significant number of samples, or there is a high level of variance in the coefficient estimates across bootstrap samples. There is `r positive` of bootstrap samples where the product of the coefficients was greater than zero, indicating a positive association between both predictors and the response variable in these samples.**
...
**This `model_1` incorporates a mix of baby's measurements at birth and maternal factors. The inclusion of maternal smoking (`smoken`) and pre-pregnancy BMI (`ppbmi`) could be particularly insightfful, as they are controllable factors that may significantly birth weight.**
...
**The plot of residuals against fitted values for `model_1` did not show any clear patterns, suggesting that the model's assumptions(linearity, homoscedasticity) were reasonably met. However, some outliers were noted, indicating specific cases where the model's predictions were less accurate.**
